...
### Character Encodings
* 인코딩이란? 말 그대로 부호화, 코드화 (암호화)한다고 생각하면 편하다. 즉 읽기 어려운 글자로 변경하는 것
* 반대로 디코딩은 인코드된 문자를 읽을 수 있도록 복호화한다고 생각하면 편하다
#### 유니코드
* 유니코드는 인코딩이 아닌 문자를 숫자로 표현하기 위한 매핑정보이다.
* 문자하나 - 숫자하나를 매핑한 정보이다. A - U+0041에, 가 - U+AC00에 매핑되어있다.
* 이 유니코드를 저장하는 방법에는 UTF-8, UTF-16등이 있다.
#### UTF-8
* 자주접하게 되는 유니코드 인코딩중 하나이다. 문자열을 8bit 기반으로 저장한다.
* 유니코드는 범위에 따라서 UTF 표현시 바이트패턴이 달라진다.
* 예를 들어 ASCII코드를 포함한 유니코드의 첫 128자리의 문자는 1바이트로,
* 한국어는 3바이트로 표현되도록 패턴이 정해져있다. `가 = U+AC00 = 44032 = 1010110000000000` 마지막 2진수를
* 3바이트 패턴`1110xxxx 10xxxxxx 10xxxxxx`에 넣으면 `11101010 10110000 10000000`가 된다.
* URL인코딩 또한 UTF-8인코딩이라고 할 수 있다.
* https://ssaemo.tistory.com/28
