...
#### 크롬 브라우저는 어떻게 동작할까?
* 컴퓨터 실행과 관련된 핵심 용어와 크롬브라우저의 다중 프로세스 아키텍쳐를 살펴본다
##### 핵심용어
* CPU: 컴퓨터의 두뇌 역할, 하나의 CPU코어는 하나의 일을 순서대로 처리할 수 있다
* GPU: 간단한 작업에만 특화되어있지만  동시에 여러작업을 수행할 수 있다.
    * CPU와 달리 제어기능, 복잡한 기능들을 제거하고 다수의 성능이 적은 ALU만 존재한다. 따라서 병렬작업이 가능하다
* 프로세스: 어플리케이션이 실행하는 프로그램
* 스레드: 프로세스 내부에서 실행되는 프로그램의 일부를 실행
* IPC: Inter Process Communication, 두 개의 프로세스가 서로 정보를 공유해야 할 때 IPC통신을 한다.
* RPC: Remote Procedure Call, 분산 네트워크 환경에서 원격지의 프로그램을 호출하여 실행할 수 있도록하는 패턴
...
##### 브라우저 아키텍쳐
* 프로세스 종류
    * 브라우저프로세스: 다른 프로세스 조정, 탭영억 밖의 모든 부분을 제어한다
    * 랜더러프로세스: 탭안에 보여지는 모든 부분을 제어한다
    * 플러그인 프로세스
    * GPU프로세스
* 크롬 브라우저 어플리케이션은 여러 프로세스를 사용한다
    * 이전에는 탭마다 프로세스 생성, 최근에는 사이트(iframe)마다 프로세스를 할당한다
    * 다수의 프로세스를 할당하면 좋은점
        * 사이트마다 격리(Sand box)를 하여 메모리 공유 문제, same origin policy를 해결할 수 있다
        * 프로세스가 crash가 되어도 프로그램으로 전파되지 않도록 한다
